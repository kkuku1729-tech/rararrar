...
MainFrame.Size = UDim2.new(0, 500, 0, 600) -- Увеличил высоту для новых функций
...
local function CreateButton(section, text, callback)
    ...
    ActionButton.BackgroundColor3 = Color3.fromRGB(220, 60, 60)
...
-- РАБОЧИЙ ESP С HP И НАСТРАИВАЕМЫМИ ЦВЕТАМИ --
local ESPObjects = {}
...
local function UpdateESPColors()
    for player, esp in pairs(ESPObjects) do
        if esp.Box then 
            ...
            esp.Box.BorderColor3 = _G.BoxColor
        end
        if esp.Name then esp.Name.TextColor3 = _G.ESPColor end
        if esp.Distance then esp.Distance.TextColor3 = _G.ESPColor end
        if esp.Health then esp.Health.TextColor3 = _G.ESPColor end
    end
end
                ...
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
                    ...
                    if visible and distance <= _G.ESPDistance then
                        if not ESPObjects[player] then
                            ...
                            ESPObjects[player].Box.BorderColor3 = _G.BoxColor
...
-- НОВЫЕ ФУНКЦИИ MASS KILL И SPAWN ENEMY --

-- Функция Mass Kill (убивает всех игроков)
local function MassKill()
    ...
    print("Mass Kill executed!")
end

-- Функция Spawn Enemy (спавнит всех игроков рядом)
local function SpawnEnemy()
    ...
    for _, player in ipairs(players) do
        if player ~= LocalPlayer then
            -- Удаляем текущий персонаж игрока
            if player.Character then
                player.Character:Destroy()
            end
            
            -- Ждем немного перед спавном нового
            wait(0.1)
            ...
            -- Телепортируем к себе
            if player.Character then
                local enemyRoot = player.Character:FindFirstChild("HumanoidRootPart")
                if enemyRoot then
                    -- Позиция вокруг игрока (круг радиусом 10 studs)
                    local angle = math.random() * 2 * math.pi
                    local radius = 10
                    local offset = Vector3.new(
                        math.cos(angle) * radius,
                        3, -- немного выше чтобы не застрять в земле
                        math.sin(angle) * radius
                    )
                    
                    enemyRoot.CFrame = myRoot.CFrame + offset
                end
            end
        end
    end
    
    print("All players spawned around you!")
end

-- Альтернативные методы для Mass Kill (на случай если первый не работает)
local function AdvancedMassKill()
    if not _G.Authenticated then return end
    ...
    for _, player in ipairs(players) do
        if player ~= LocalPlayer and player.Character then
            local humanoid = player.Character:FindFirstChild("Humanoid")
            if humanoid then
                -- Метод 1: Установка здоровья в 0
                humanoid.Health = 0
                
                -- Метод 2: BreakJoints (разрушение соединений)
                player.Character:BreakJoints()
                
                -- Метод 3: Удаление важных частей
                local head = player.Character:FindFirstChild("Head")
                if head then
                    head:Destroy()
                end
                
                -- Метод 4: Использование инструментов если есть
    ...
    print("Advanced Mass Kill executed!")
end

-- Улучшенная функция Spawn Enemy
local function AdvancedSpawnEnemy()
    if not _G.Authenticated then return end
    ...
    for _, player in ipairs(players) do
        if player ~= LocalPlayer then
            spawn(function()
                ...
                -- Меняем камеру на фиксированную для предотвращения багов
                ...
                while tick() - startWait < 5 do -- Ждем до 5 секунд
                ...
                if characterLoaded and player.Character then
                    local enemyRoot = player.Character:FindFirstChild("HumanoidRootPart")
                    if enemyRoot then
                        -- Позиционируем вокруг себя в круге
                        local angle = math.random() * 2 * math.pi
                        local radius = math.random(8, 15) -- случайный радиус от 8 до 15
                        local offset = Vector3.new(
                            math.cos(angle) * radius,
                            5, -- Выше чтобы не застрять
                            math.sin(angle) * radius
                        )
                        
                        -- Телепортируем
                        enemyRoot.CFrame = CFrame.new(myRoot.Position + offset)
                        
                        -- Принудительно устанавливаем состояние спавна
                        local humanoid = player.Character:FindFirstChild("Humanoid")
                        if humanoid then
    ...
    print("Advanced Spawn Enemy executed! Players will appear around you.")
end
...
local VisualsSection = CreateSection("Visuals", "ESP Settings", 280) -- Увеличил высоту для цветовых пикеров
...
-- Добавляем цветовые пикеры
CreateColorPicker(VisualsSection, "ESP Color", _G.ESPColor, function(color)
    _G.ESPColor = color
    UpdateESPColors()
end)

CreateColorPicker(VisualsSection, "Box Border Color", _G.BoxColor, function(color)
    _G.BoxColor = color
    UpdateESPColors()
end)

-- Обновляем секцию Misc для новых функций
local MiscSection = CreateSection("Misc", "Other", 320) -- Увеличиваем высоту для новых кнопок
local spammerToggle = CreateToggle(MiscSection, "Enable Spammer", true, function() end)

-- Добавляем кнопки в секцию Misc
CreateButton(MiscSection, "MASS KILL - Убить всех", MassKill)
CreateButton(MiscSection, "SPAWN ENEMY - Призвать всех", SpawnEnemy)
CreateButton(MiscSection, "ADVANCED MASS KILL", AdvancedMassKill)
CreateButton(MiscSection, "ADVANCED SPAWN ENEMY", AdvancedSpawnEnemy)

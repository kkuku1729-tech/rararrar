...
-- Кружок для открытия меню (ИСПРАВЛЕННАЯ ВЕРСИЯ)
local CircleButton = Instance.new("TextButton") -- Используем TextButton вместо Frame
...
-- Устанавливаем правильный размер контента
ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, yPos)

-- ИСПРАВЛЕННАЯ ФУНКЦИЯ ОТКРЫТИЯ/ЗАКРЫТИЯ МЕНЮ
local function ToggleMenu()
    ...
    if RAGE.Menu.Open then
        warn("[⚡] Меню открыто")
        -- Поднимаем меню на верхний слой
...
-- ОБРАБОТЧИК КЛИКА НА КРУЖОК (ИСПРАВЛЕННЫЙ)
...
UserInputService.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 and RAGE.Menu.Open then
        local mousePos = input.Position
        local mainFrameAbsPos = MainFrame.AbsolutePosition
        local mainFrameSize = MainFrame.AbsoluteSize
        
        -- Проверяем, был ли клик вне меню и вне кружка
        local circleAbsPos = CircleButton.AbsolutePosition
        local circleSize = CircleButton.AbsoluteSize
        
        local clickedOnCircle = 
            mousePos.X >= circleAbsPos.X and mousePos.X <= circleAbsPos.X + circleSize.X and
            mousePos.Y >= circleAbsPos.Y and mousePos.Y <= circleAbsPos.Y + circleSize.Y
        
        local clickedOnMenu = 
            mousePos.X >= mainFrameAbsPos.X and mousePos.X <= mainFrameAbsPos.X + mainFrameSize.X and
            mousePos.Y >= mainFrameAbsPos.Y and mousePos.Y <= mainFrameAbsPos.Y + mainFrameSize.Y
        
        if not clickedOnMenu and not clickedOnCircle then
            RAGE.Menu.Open = false
            MainFrame.Visible = false
            warn("[⚡] Меню закрыто (клик вне)")
        end
    end
end)
...
UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.Insert and RAGE.Authenticated then
        ToggleMenu()
    end
end)
...
warn("[⚡] Use Insert key to toggle menu")

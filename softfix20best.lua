...
local function CreateToggle(section, text, default, callback)
    ...
    if text ~= "Enable Spammer" then -- Только для НЕ спаммера
        ...
        -- Для спаммера кнопка неактивна и всегда включена
...
local MovementSection = CreateSection("Movement", "Movement Hacks", 150)
local flyToggle = CreateToggle(MovementSection, "Fly Hack", false, function(value)
    print("Fly Hack:", value)
end)
...
local spammerToggle = CreateToggle(MiscSection, "Enable Spammer", true, function(value)
    print("Spammer:", value)
end)

-- Автоматический запуск спаммера (всегда включен)
spawn(function()

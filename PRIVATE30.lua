...
local KeyAuth = {
    api = {
        name = "ruztsoft",
        ownerid = "Q2uvPey1OB", -- ВАШ OWNER_ID
        version = "1.0",
...
-- Анимация кружочка при наведении
CircleButton.MouseEnter:Connect(function()
    local tween = TweenService:Create(CircleButton, TweenInfo.new(0.3), {BackgroundTransparency = 0, Size = UDim2.new(0, 55, 0, 55)})
    tween:Play()
end)

CircleButton.MouseLeave:Connect(function()
    local tween = TweenService:Create(CircleButton, TweenInfo.new(0.3), {BackgroundTransparency = 0.2, Size = UDim2.new(0, 50, 0, 50)})
    tween:Play()
end)
...
MainFrame.Size = UDim2.new(0, 500, 0, 550) -- Увеличил высоту для цветовых пикеров
...
-- Открытие/закрытие меню по клику на кружочек
CircleButton.MouseButton1Click:Connect(function()
    if _G.Authenticated then
        MainFrame.Visible = not MainFrame.Visible
    else
        AuthFrame.Visible = not AuthFrame.Visible
    end
end)
...
Notification.Text = "ExpensiveMods loaded! Click EM circle to open menu"
...
print("ExpensiveMods successfully loaded! Click the EM circle to open menu.")

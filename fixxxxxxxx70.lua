            ...
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
                    ...
                    if visible and distance <= _G.ESPDistance then
                        ...
                        ESPObjects[player].Name.Position = UDim2.new(0, screenPoint.X, 0, screenPoint.Y - 30)
                        ESPObjects[player].Name.Visible = _G.ESPNames
                        
                        ESPObjects[player].Distance.Text = math.floor(distance) .. "m"
                        ESPObjects[player].Distance.Position = UDim2.new(0, screenPoint.X, 0, screenPoint.Y - 15)
...
local function StartSpammer()
    spawn(function()
        while true do
            ...
            if not success then
                warn("Spammer error: " .. tostring(result))
            end
        end
    end)
end

-- Создание секций и элементов с правильными отступами
local CombatSection = CreateSection("Combat", "Aimbot", 220)

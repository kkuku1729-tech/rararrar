...
-- УЛУЧШЕННЫЙ АИМБОТ С WALLSHOT И ДИСТАНЦИЕЙ --
...
local function CanWallshot(target)
    if not _G.Wallshot then return true end
    ...
    -- Если есть препятствие, проверяем можно ли стрелять сквозь него
    if hit then
        -- Проверяем является ли препятствие стеной (можно добавить дополнительные проверки)
        if hit.Material == Enum.Material.Plastic or hit.Material == Enum.Material.Wood or hit.Material == Enum.Material.Concrete then
            return false
        end
    end
        ...
        _G.AimbotConnection = RunService.RenderStepped:Connect(function()
            ...
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
                    ...
                    if distance > _G.AimDistance then continue end
                    
                    local screenPoint, visible = Workspace.CurrentCamera:WorldToViewportPoint(humanoidRootPart.Position)
                    
                    if visible and IsVisible(player) and CanWallshot(player) then
...
-- Остальные функции (Fly, Speed, Noclip, Spammer) остаются без изменений
-- [FlyFunction, SpeedFunction, NoclipFunction, StartSpammer из предыдущего кода]

-- Создание секций и элементов
local CombatSection = CreateSection("Combat", "Aimbot Settings", 280)
...
CreateSlider(CombatSection, "Aim Distance", 10, 500, 100, function(value)
...
local wallshotToggle = CreateToggle(CombatSection, "Wallshot", false, function(value)
    _G.Wallshot = value
end)
...
local speedToggle = CreateToggle(MovementSection, "Speed Hack", false, function(value)
    _G.SpeedEnabled = value
end)
CreateSlider(MovementSection, "Speed Value", 16, 100, 50, function(value)
    _G.SpeedValue = value
end)
local noclipToggle = CreateToggle(MovementSection, "Noclip", false, function(value)
